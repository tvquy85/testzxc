...
## 2026-06-21 Clean V4 Medium Recovery Notes

- Local NLI grounding can use `sentence_transformers.CrossEncoder("cross-encoder/nli-deberta-v3-small", local_files_only=True)` from the LOBExp venv. This is faster and cleaner than lexical fallback, and Step09 medium records `nli_loader=sentence_transformers_cross_encoder`.
- Forecast prediction should treat the LLM distribution as the model evidence and compute the trading `action` deterministically from that distribution. The raw model action is preserved as `raw_action`, while the official action uses `down_side`, `up_side`, and `neutral` according to the prompt policy. This avoids silently accepting raw `hold` actions when the probability distribution is clearly directional.
- The medium recovery run validates mechanics but not final paper performance: Step15 prediction schema passed, Step16 daily backtest passed mechanically with 146 trading days but had negative Sharpe, and the strict science gate returned `GO_MEDIUM` plus `CLAIM_RESTRICTED`.
- Step18 now includes a real Qwen RWSFT baseline on the same 300-row test window before comparing against the DPO adapter. PEN, SEP, and Policy codebases are listed as `reference_only=true` because comparable current-data medium runs were not executed, so no outperforming claim is allowed against them.
- Step17 counterfactual evidence is treated as a pipeline PASS but not a scientific claim: overall pass rate was below the strict 0.50 threshold, and news evidence perturbations did not reach the required per-type pass rates.
