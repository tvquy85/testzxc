...
def resolve_hf_path(path):
    if os.path.exists(os.path.join(path, "snapshots")):
...
def main():
    ...
    parser.add_argument("--base_model", required=True, help="Path to base model (e.g. Llama-3-8B)")
    parser.add_argument("--lora_path", default=None, help="Path to LoRA adapter (e.g. FinGPT)")
    ...
    base_model_path = resolve_hf_path(args.base_model)
    ...
    if args.lora_path:
        lora_resolved = resolve_hf_path(args.lora_path)
        print(f"Loading LoRA weights from {lora_resolved}")
        model = PeftModel.from_pretrained(model, lora_resolved)
    
    model.eval()
    
    print("3. Generating Rationales...")
    rationales_text = []
    batch_size = 4
    ...
    for i in tqdm(range(0, len(generator_prompts), batch_size), desc="Generating"):
        ...
        for msg in batch:
            try:
                texts.append(tokenizer.apply_chat_template(msg, tokenize=False, add_generation_prompt=True))
            except ValueError:
                text = ""
                for m in msg:
    ...
    print("4. Evaluating with Llama-3 Proxy Judge...")
